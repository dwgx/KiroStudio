...
- W1–W6 神文件 `#[path]` 抽取；MCP 直连 401 同请求换号；SSO 空 refreshToken 拒入池。
- P0-1 Bug C：required 校验在映射前（Kiro 形态）；内置工具流式不再误杀。
- P0-3 `config/import` 继承 `config_path`；P0-4 quota 告警；P0-5 persist 串行；P0-6 sniff thinking 先 stop。
- P1 记账/安全/CI；P2 ciborium、toolchain 1.97.1、batch API、insightCode、OpenAI 会话头、vitest。
- D：deploy-build 钉 1.97.1；settings toForm/diff；SSE scale 头；bg-img 同源+限流；启动清 `.tmp`。
- E：`balance_cache` / `socks_nodes` / `config_update` / `service_restart` `#[path]`。
- F：透传池 `support_rank` 正向路由（有意行为变化）；GPT 族 `reasoning.effort`。
- G：全池配额 402；纯透传池跳过 WebSearch MCP；`POST /api/admin/models/test`；HTTP-date Retry-After；CJK token 4.5；JSON 源/帮助页搜索 SSRF 守卫。

## [1.1.2] - 2026-08-20 - W15–W22 收口 + 吸收/调度

相对 v1.1.1 / `59744cb`。Actions 三端绿。本机当时 **2170 passed**。
...
代理池自动健康调度、KAM 导出、接入信息页等 20+ 项变更。

### 选号与性能

- **token_manager 选号临界区优化**：custom rpm 批量预取、health 读路径去写、report_success
  族键缓存、refresh_lock 细化为 60s 级别，降低并发选号下的锁竞争与写放大。
- **模型级限流**：`model_hits` 计数 + 白名单路由软因子，模型维度超限提前收敛而非全池连坐。
- **count_tokens 缓存**：外部 count_tokens API 结果缓存，避免重复计费往返。
- **SSE 预分配**：流式响应缓冲预分配，减少分配抖动。
- **trace 批量写**：`insert_batch` 批量入库 + SQLite `busy_timeout=5000`（多实例并发写不假死）
  + `interrupted_bytes` 中断字节数记录。

### 可靠与安全

- **config 三写路径同锁**：配置多路径写入统一锁口径，消除并发覆盖。
- **cooldown 持久化**：冷却状态带 `PersistState` 落盘，重启后冷却不丢。
- **告警 webhook**：新增 `alerting.rs`，8 个接入点触发 webhook 推送，连续失败冷却防轰炸。
- **403 封禁识别**：按响应文本词族识别 403 封禁（`temporar` 词族排除误判），封禁凭据正确进熔断。
- **CSP + sessionStorage**：管理面补 CSP 响应头（script 仅同源）+ `X-Content-Type-Options`；
  adminKey 从 localStorage 明文改存 sessionStorage（关标签即清，不留历史残留副本）。
- **备份轮换**：凭据/配置备份轮换策略落文档与实现（`docs/SECURITY-BACKUP.md`）。
- **超额自动禁用空 breakdown 门**：超额自动禁用前校验 breakdown 非空，防空数据误禁。
- **websearch 预算耗尽收尾**：预算耗尽时正确收尾流式响应，不留悬挂流。

### 新能力

- **成本核算**：`model_pricing` 配置化定价，按模型核算成本入 trace。
- **代理池自动健康调度**：代理按健康状态自动调度，坏代理自动退出选路。
- **诊断快照端点**：一键导出运行态诊断快照。
- **OTA 自动检查**：`ota_auto_check` 后台定时检查（默认关，间隔 24h 可配）。
- **版本伪装**：从官方元数据端点取 `currentRelease` 构造与官方 IDE 一致的 UA 版本段；
  用量类接口（refresh/区探测）刻意固定 `kiro_version` 段，规避新版 profileArn 强求。
- **cli_ua 收敛**：CLI 端点 UA 组合收敛为最多 3 个。
- **KAM 导出**：KAM 凭据批量导出。
- **/healthz**：健康检查端点，供部署探活。

### 前端

- **接入信息页**（conn-page）：展示接入地址/密钥/端点组合等接入所需信息。
- **掩码点击复制完整 Key**：凭据卡掩码展示，点击即复制完整 Key。
- **设置页三开关**：新增三个配置开关入口。
- **画布右键菜单**：凭据画布右键快捷操作。
- **OTA 进度**：升级进度可视化。
...
- 测试 **1760 passed / 0 failed**（`--no-default-features`）；release 5 产物；nbus 已部署。

## [1.0.1] - 2026-08-13 - 恢复版本号 + Cargo.lock 依赖版本修复 + CI test 去 --locked

1.0.0 发布后，本机压测临时 bump 的版本号在后续 lock 重建中被覆盖，本版恢复版本号并
修复发布链：Cargo.lock 依赖版本还原、CI test job 去 `--locked`、CI 固定 Rust 版本。

### 版本号恢复

- **本机压测版临时 bump**（97b85ef）：本地压测时把版本批量替换为 1.0.1（Cargo.toml +
  Cargo.lock 根包），**误伤** lock 内依赖的版本号（anstream 等被一并替换）。
- **多轮 lock 修复**：475151f 恢复被误改的依赖版本（anstream 等）；0f0e570 用
  `cargo generate-lockfile` 重建（anstream/http-body 版本一致性）；9951297 恢复原始依赖
  版本（仅根包 1.0.1）；后续按 CI 同版本/官方源/固定 Rust 多次重建 lock 以保可复现。
- **正式恢复版本号**（7368a70）：Cargo.toml 恢复为 1.0.1 正式发布。

### CI 修复

- **test job 去掉 `--locked`**（f47f812）：crates.io yank 环境下 lock 会假红；编译本身
  即可验证依赖兼容性。
- **固定 Rust 1.97.1**（f9c0757）：lock 可复现，不再随 stable 滚动。

### 模型探测增强

- **智能剥离候选**（cc49efe）：模型探测剥离 `/v1`、`/anthropic` 组合候选；前端 base_url
  变更后旧探测结果失效，不再复用过期结果。

## [1.0.0] - 2026-08-13 - 首个大版本，全链路就绪

从 0.7.47 起的批次汇聚为首个大版本：网关核心（共享重试预算/ThrottleProfile/cache 指纹/
端点健康与模型映射）、admin 层与前端扩展、SSRF 修复、端点换桶与 amazonq/codewhisperer
端点接入，版本线 0.7.47 → 0.7.48（nbus 部署目标）→ 1.0.0。

### 网关核心（0.7.47）

- **共享重试预算**：重试预算跨请求共享，收敛重试放大。
- **ThrottleProfile 预设**：限流档位预设化，按场景一键切档。
- **cache 指纹**：新增 `cache_fingerprint.rs` 缓存指纹计算（透传/归一化共用）。
- **端点健康与模型映射**：端点健康状态跟踪（`endpoint_health.rs`）与模型映射
  （`model_mapping.rs`），探测/路由解耦。
- **透传吸收覆盖**：passthrough 吸收上游响应差异（`passthrough.rs` 大幅扩展）。
- 转换/流式/WebSearch 层配套大改（converter、handlers、stream、websearch）。

### 端点层

- **端点换桶重构**（42df1d2）：已 hotswap 到生产（当时未提交的防丢失 snapshot）。
- **amazonq / codewhisperer 端点接入**：新增两个 IDE 端点实现。

### admin 层与前端（0.7.48）

- **用量统计扩展**：usage_stats/record 扩展，用量口径细化。
- **throttle 档位接线 + 上号端点**：reprobe / 超额禁用 / refresh-token / relogin 四个
  上号运维端点；自愈参数配置化（`config.rs` 大幅扩展）。
- **admin-ui**：dashboard/settings/usage 扩展、三语 i18n 补全、档位选择器、死配置提示。

### 修复与文档

- **SSRF 修复**（ba2f8fa）：`AdminConfigured` 放行字面量环回（本机服务互转），内嵌
  混淆地址仍拒。
- **文档**（8e97434）：架构决策、websearch 缓冲设计（未实施）、透传兼容规格、限流旋钮
  实测结论、状态快照。
...
**IP 黑名单(反代安全新增)**:运维台设置→反代安全新增「IP 黑名单(封禁)」——命中 CIDR/单 IP 即拒绝
访问(403),**优先于白名单判定**。用于封禁特定滥用 IP。复用现有 CIDR 解析基础设施(IpBlocklist,
参照 IpAllowlist);入口中间件先判黑名单(命中即拒)再判白名单;config 字段 `ip_blocklist`(整表替换,
保存校验 CIDR 合法性,restart-only)。前端三语 + 2 个新单测。

**I18N 模块级残留清扫**:真机切英文后仍显示的中文——运维指标卡(METRIC_ITEMS)、熔断态(CIRCUIT_META)、
设备(DEVICE_META)、结果(OUTCOME_META/LABEL)、时间格式(timeAgo/formatLastUsed)、号池 tooltip
(当前活跃/在途)等**模块级常量/函数**里的中文——改成渲染时 t() 或 i18n 单例(每次调用取当前语言),
切语言实时更新。三语字典扩到 1453 key。

## [0.7.35] - 2026-07-16

### I18N 三语覆盖全站 + 语言切换器改分段按钮 + 搜索关键词三语化

**语言切换器改分段按钮**:原下拉式在侧边栏 footer 会往下弹出跑到视口外,改成三个紧凑按钮
(中/EN/日)+ 滑动高亮块弹性动画(cubic-bezier 回弹),按压即切。

**I18N 三语覆盖扩到全站**(接 0.7.34 的 13 核心组件):新增国际化 app-shell(侧边栏菜单)/
idc-login/social-login/region-switcher/balance-dialog/batch-verify/diagnosis-card/proxy-test-button/
**settings-page 全分区**(服务/存储/RPM/隐私/令牌导出/回收站/UI排版/负载均衡/智能调度/工具容错/
防关联限流/网络上号/登录背景/反代安全/token预刷新 等)。三语字典 945 → **1387 key**(zh/en/ja 一致)。

**设置搜索关键词三语化**:SectionGate 搜索从内联中文关键词数组重构为 i18n key(`*.kw`),关键词随
界面语言切换,英文界面可用英文关键词搜设置项;并补齐 CARD_INDEX 缺失的智能调度分区(命中计数修正)。
...
真机坐实"RPM 跑太多就凭据不流通,关 all_cooling_fast_fail 也没用"——真凶是**入站整形排队超时**(非冷却):
单号被上游 429 砍到下限 RPM 后,请求量远超该 RPM → 大量请求在 `throttle.acquire()` 排队 → 超过
`queue_max_wait`(30s)→ 返回 429 拒绝 → 请求没到上游就被网关自己卡死超时 = 不流通。all_cooling_fast_fail
管的是冷却路径,够不到这条,故关它无效。

- 新增配置 `inbound_queue_timeout_passthrough`(默认 **true**):排队超时后**放行**去打上游,而非返回 429。
  入站整形平峰仍削峰限速,只在真堆积超时时降级为放行——**最坏退化成不限速,绝不再"不流通"**。
  关=旧行为(超时返回 Retry-After 429,多号池/需严格保护上游时用)。
- 运维页「入站整形」齿轮卡新增「排队超时放行」开关(热更即时生效)。后端 config 字段(默认 true,持久化)
  + admin 响应/update 接线 + throttle acquire 超时分支按开关放行/拒绝。
- 2 个新测试锁两种行为(超时放行 Ok / 超时拒绝 Err)。后端 763 绿,前端 build 绿。

## [0.7.32] - 2026-07-16

### 运维页 UI 增强 + 全池冷却快速失败开关

**全池冷却快速失败开关(运维页可配,治单号频繁 429)**:
- `all_cooling_fast_fail` 从"写死默认 true、前端改不了"提升为**运维页「防关联/限流」分区可配开关**(热更即时生效)。
- 开(默认,多号池合适):全池都在冷却时立即返回 429+Retry-After 让客户端退避。
- 关(单号/小池更平滑):网关内短等到有号恢复再重试,避免唯一的号一有几秒短冷却就秒拒 429(体感"不回复")。

**运维页/凭据管理 UI 增强**:
- 用量日志:「按模型」「按号」明细表格改为各自独立滚动区(max-h + 表头 sticky 吸顶),长表格不再撑长弹窗/被截断。
- 登录背景图缓存:①放大态按 ESC 只收起放大层、不关整个弹窗(拦截 Radix Dialog 默认关闭);②Ctrl/⌘+单击
  勾选多张图,顶部工具栏批量下载 / 清空,选中图高亮。
- 凭据管理:勾选多个凭据后,批量操作工具栏新增「导出选中」按钮,复用后端 export 端点逐个拉取打包成一个 JSON
  下载(格式与设置页「导出全部」一致)。`downloadJson`/`fileStamp` 抽到 `@/lib/utils` 共用。

## [0.7.31] - 2026-07-16

### 修 RPM 自动挡升档饿死死锁(根治"跑着跑着不调度、必须重启网关")

真机日志坐实:AIMD 自动挡 RPM 被上游 429 一路砍到下限(`300→150→75→37→20`)后,3 小时内**零升档**,
卡死在 20 RPM(极低≈不调度),必须重启网关(清零状态)才恢复——正是 dwgx 反馈的"莫名其妙不调度了,
每次都要重启"。

- **根因**:`report_upstream_429` 的去抖分支**错误刷新了 `last_md_nanos`**(上次降档时刻)。该字段被
  `maybe_step_up` 用作升档静默期判据(距上次降档 ≥20s 才升档)。上游只要**持续零星 429**(哪怕 RPM 已在
  下限、砍无可砍、429 全被去抖挡掉),`last_md_nanos` 就被反复刷成 now → 升档的"≥20s 静默"永不满足 →
  RPM 死锁在下限不回升。重启清零 `last_md_nanos` 才暂时恢复,直到又被打到下限卡死。
- **修复**:去抖分支**不再刷新** `last_md_nanos`——它现在纯粹是"上次**真正降档**"的时刻。去抖语义不变
  (一波 failover 链的多次 429 仍落在同一 3s 窗、只降一档),而持续 429 不再污染升档静默期 → RPM 正常回升。
- 改动仅一行(删去抖分支里的 `b.last_md_nanos = now`)。新增回归测试 `test_debounced_429_does_not_
  refresh_last_md_no_upshift_starvation`(**已在旧代码上验证必失败**,非走过场)。全量 761 绿。
