...
> ## ✅ 2026-08-06 状态更新：EXP-0 / EXP-1 / EXP-2 的核心问题**已全部回答，不要重跑**
>
> | 实验 | 状态 | 结论 / 去处 |
> |---|---|---|
> | **EXP-0** | ✅ 已做并已上线（2026-07-28） | **上游不发 prompt cache 真值**（`metadataEvent` 只有 `stopReason`）⇒ **L2-2 整个不用做**。详见 `docs/CACHE-EXP0-RESULT.md` |
> | **EXP-1 / EXP-2** | ✅ **核心问题已回答**（2026-08-05/06，用线上 **46 万条 traces**，不是探针） | **上游没有可观测的隐式前缀 credit 折扣 ⇒ 分叉 C 成立。** 详见 **`docs/prefix-stability-2026-08-06.md` 第 1 节** |
>
> **两条"矛盾"证据已收口**：「无隐式缓存」结论成立但原方法学是错的；
> **下文多处引作基线的「47% credit 折扣」（`converter.rs` 那次 0.141→0.075）是不可复核的
> 孤例观测** —— 单次、无样本数、落在正常噪声内（固定 model+input+output 后 credit
> 组内极差平均 **49.06%**）。⇒ **本文件凡以「47% 折扣」为收益锚点的段落（含 EXP-2 H 组、
> 分叉 B 的收益估算）都不再有效**，别再用它当任何决策依据。
>
> 仍然可做的是**其余组次**（credit 粒度本身、TTL 探测、G 组显式 cachePoint 是否净亏）。
> 动手前先读本文件的探针预算控制（专用测试凭据、QPS ≤ 0.2、遇 429 停 5min、
> 总量 ≤ 350 请求 ≈ 70 credits）。
>
> **优先级后果**：分叉 C 既已证实，**L3（网关自建响应缓存）升为最高优先**（上游不给折扣时
> 它是唯一省钱手段），**L0/L1 的价值退化为纯 token 节省 + 会话亲和稳定**。
> 已做的 L0 改动**不要回滚** —— 它们的 token 节省与亲和收益仍成立，只是原始依据已作废。
...
**修法**：
- 触发点后移，`trigger_bytes` 默认从 4MiB 提到 **12MiB**（原稿写 4.8MiB，已被实测推翻，见下）
...
### ⚠️ 2026-08-04 实测修正：原稿的 4.8MiB 与「约 5MiB 硬限」都不成立

本节原写「触发点后移到尽可能接近上游硬限（实测约 5MiB），阈值提到 4.8MiB」。生产实测
（24 小时、3,313 次压缩事件）否掉了这两个前提：

**① 4.8MiB 只能救 9.1% 的请求。** 压缩前大小分布：

| | 最小 | p25 | 中位 | p75 | 最大 |
|---|---|---|---|---|---|
| 压缩前 | 4.00M | 5.49M | **6.93M** | 10.03M | 14.32M |

只有 9.1% 落在 4–4.8MiB 区间，其余 90.9% 本来就远超 4.8MiB，提到 4.8MiB 照样触发。

**② 上游硬限远高于 5MiB。** 88% 的请求**压缩后仍 > 5MiB**（中位 6.92M，最大 14.21M），
而同期上游 400 **为零**。所以「约 5MiB 触发 400」这条记载不成立，硬限至少 ≥14MiB
（上界仍未知——只能证明 14.32MiB 成功过，不能证明更大也行）。

**③ 压缩收益可忽略，代价是前缀反复失效。** 节省量中位 0.01M（占 6.93M 的 **0.14%**）。
`saved_bytes` 只有 30 个唯一值，最高频的是 **4 字节，出现 858 次** —— 为 4 个字节改写整个
请求体 858 次，每次都可能让上游前缀缓存失效。

**结论**：阈值应提到 **12MiB**（实测最大 14.32MiB 成功过，留余量），而非 4.8MiB。这样 88%
的请求不再被无意义改写，同时保留对真正超大请求的防护。

**为何不直接 `compression.enabled = false`**：三条保留意见 ——
(a) 只验证了 24 小时，若某天真有超限请求，压缩是唯一防线，关掉会变成 400 直接失败；
(b) `saved_bytes=103439` 那一档（327 次）是真在压大 `tool_result`，有实际作用；
(c) 硬限只能证明 ≥14MiB，无法证明上界。
...
## 全站命中率的三个上限（2026-08-04 实测，08-03 全天 48,402 请求）

**「100% 命中」不可达。** 三个互相独立的上限是**乘法**关系，任何单项修复都被其余两项压住：

| 上限 | 实测值 | 成因 |
|---|---|---|
| 会话键可命中占比 | 88.3%（保守）/ 98.4%（乐观） | L0-5 归并后；未修时仅 65.8% |
| 号粘性（同会话同号） | 96.7% | 上游缓存按号隔离，换号必然未命中 |
| 稳态前缀稳定性 | 75.3% | 1 − 实测前缀塌陷率 24.71% |
...
| 现状（什么都不修） | **47.9%** |
| 只修 L0-5 | **64.3%**（+16.4 点） |
| L0-5 + 修前缀塌陷（L0-2/3/4） | **85.4%**（再 +21.1 点） |
| 乐观口径上界 | 95.2% |

**两条对本文档其余部分的修正**：

**① 会话键归并的 98.4% 是虚高，保守口径 88.3%。** 用 device+model+IP 做代理键归并后校验，
组内 5 分钟内的相邻请求只有 **69% 的 gross 单调延续**（同一上下文），31% 出现跳变（比值中位
0.511）—— 无关对话被混进同一个键。真实的 `SHA256(system+tools)` 会切得更细，实际落点应在
88.3%–98.4% 之间，需上线后实测。

**② L0-5 有一个固有局限：多用户共享同一代理键。** 实测最大一组 12,500 个请求全部来自
`143.20.230.248`（即 sub2api 转发），相邻 gross **47% 在下降**（中位 −156,438）——多个独立
对话交错的铁证。sub2api 转发时所有用户共享 `client_device`/IP，若他们的 system+tools 也相同
（同一批 Claude Code 用户很可能如此），`SHA256(system+tools)` 仍会归到一个键，后果是**缓存
互相踩踏**，可能比随机键更差。根治需 sub2api 透传用户标识（如 `X-Real-User`）并加进哈希 ——
跨两个系统的改动，尚未实施。

**③ 号粘性不是瓶颈。** 全天用到 39 个号，每会话中位用 3 个、最大 24 个，但相邻请求同号率
96.7% —— 现有调度已把同会话大多留在同号上，这层几乎不用管。
（注意口径：`gateway-status` 报的 `pool=1/1` 是 sub2api 侧通道数，与 KiroStudio 侧 39 个号
不是同一回事。）
...
| 技术 | L0-4 取消图片跨历史去重导致请求体变大撞上游大小硬限 | `MAX_TOTAL_IMAGES=20` 已有上限；L0-2 的压缩兜底仍在。**该风险比原估低**：实测硬限 ≥14MiB 而非 5MiB（见 L0-2 修正节），当前 p75 仅 10.03MiB |
