...
> ⚠️ **避坑情报（2026-06-30 实测 M-JYuan 源码）**：4 个防关联模块均**自包含**（零 crate 内部依赖），
> 但 **fingerprint 在 M-JYuan 是未接线的半成品**——Fingerprint 生成并存进 CredentialEntry，
> 却只在 token_manager.rs:2932 被 clone（给 admin 展示），其 `user_agent()`/`os_string()` 等
> 注入方法在非测试代码中**从未被调用**。M-JYuan/基线的出站 UA 头仍是写死的
> `aws-sdk-js/1.0.34 ... KiroIDE-{kiro_version}-{machine_id}`，并未注入多维指纹。
> → fingerprint 要真正生效，须自己研究上游接受的 UA/头格式并**抓包验证**，属"需验证外部行为"。

**移植顺序（据上修正）：**
- **第一批·纯本地调度，立即有效**（不依赖上游格式，可独立编译验证）：
  cooldown + rate_limiter + affinity —— ✅ **现在就做**
- **第二批·稳定性**：compressor + truncation + background_refresh + redact
- **第三批·需验证外部行为**（与 device flow 一起，抓包确认上游格式后再接线）：
  fingerprint（真正注入 UA/头）
- **运营面（阶段二/三）**：trace_db + usage_stats + proxy_pool + client_keys + groups
