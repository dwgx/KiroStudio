...
│ 5. 调度层       亲和 · 13键升序选号 · 重试/故障转移         │ ← kiro/provider.rs + token_manager.rs
...
客户端 POST /v1/messages
 ...
 │   │   ├─ 13 键升序选号（见 §四）；priority 模式同一套键、prio_first 恒 true
...
历史文档写的 `(priority, consecutive_failures)` 双键**已过时**。当前是 **13 键升序** `min_by_key`（权威定义在 `token_manager.rs` 排序键闭包注释，改元组必须同步那里）：
...
排序键（升序，越小越先选）：
 ...
 ⑫ success_count       —— 终身成功数
 ⑬ priority_tiebreaker —— 优先级末位兜底（关时恒 0）
```
