...
//! - **CSPRNG 密钥文件**:同目录 `.at_rest.key`，OS CSPRNG 生成 32 字节，`create_new`(O_EXCL)
//!   原子创建。密钥持久化、不随 hostname / os_user / 启动方式漂移（换机器拷走密文但
//!   **不带走密钥文件**仍解不开）。导出/导入接口走的是明文,与本加密**完全无关**。
...
//!
//! # 边界(诚实)
//! 密钥文件不是抗本机攻击者(能读 `.at_rest.key` 就能解密)——它挡的是"凭据文件离开本机而
//! 密钥没带走"(拷盘/误传/备份泄露)。要抗更强威胁需 HSM/OS keychain,超出自托管单机范围。
