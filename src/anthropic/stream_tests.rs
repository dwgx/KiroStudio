    ...
    /// 假名/韩文必须走 CJK 密度（~1.5 字/token），不能当拉丁 4 字/token。
    #[test]
    fn estimate_tokens_counts_kana_as_cjk_density() {
        let latin = estimate_tokens("abcdefgh");
        let kana = estimate_tokens("あいうえおかきく");
        let han = estimate_tokens("你好你好你好你好");
        assert!(
            kana > latin,
            "hiragana must use CJK density, not latin 4 char/token: kana={kana} latin={latin}"
        );
        assert_eq!(
            kana, han,
            "kana and hanzi of same length must share CJK density: kana={kana} han={han}"
        );
        let hangul = estimate_tokens("한글한글한글한글");
        assert_eq!(hangul, han, "hangul syllables must share CJK density");
    }

    /// pct=1000 不得把计费 input 写成 10 倍窗口；钳到 200%。
    #[test]
    fn context_input_tokens_from_pct_1000_does_not_10x() {
        let window = 200_000;
        let tokens = context_input_tokens_from_pct(1000.0, window)
            .expect("pct=1000 is finite and positive");
        assert_eq!(
            tokens,
            (200.0 * window as f64 / 100.0) as i32,
            "pct=1000 must clamp to 200% of window, got {tokens}"
        );
        assert_ne!(
            tokens,
            (1000.0 * window as f64 / 100.0) as i32,
            "unclamped 10x window must not be used"
        );
        assert_eq!(context_input_tokens_from_pct(100.0, window), Some(window));
        assert_eq!(context_input_tokens_from_pct(0.0, window), None);
        assert_eq!(context_input_tokens_from_pct(f64::NAN, window), None);
        assert_eq!(context_input_tokens_from_pct(f64::INFINITY, window), None);
    }

    /// 两个未关闭块的 stop 顺序必须按 index 稳定（不随 HashMap 迭代漂）。
    #[test]
    fn close_open_blocks_two_open_blocks_stable_order() {
        let mut manager = SseStateManager::new();
        manager.handle_content_block_start(1, "text", json!({"type": "content_block_start", "index": 1}));
        manager.handle_content_block_start(0, "thinking", json!({"type": "content_block_start", "index": 0}));
        let stops = manager.close_open_blocks();
        let indices: Vec<i64> = stops
            .iter()
        ...
        assert_eq!(
            indices,
            vec![0, 1],
            "stop order must be ascending block index, got {indices:?}"
        );
    }
    ...
    /// P0-6：嗅探路径开 thinking 块后直接 toolUseEvent（无 `</thinking>`、无 reasoning 流）。
    /// 必须先 stop thinking，再 start tool_use；关块顺序与 reasoning 分支相同。
    #[test]
    fn sniff_thinking_open_then_tool_use_emits_stop_before_tool_start() {
        let mut ctx = StreamContext::new_with_thinking("test-model", 1, true, HashMap::new());
        let mut all = ctx.generate_initial_events();
        // 嗅探开块、无闭标签：内容已作为 thinking_delta 下发，buffer 通常为空。
        all.extend(ctx.process_assistant_response("<thinking>\nplanning the call"));
        assert!(
            ctx.in_thinking_block,
            "嗅探路径必须已进入 thinking 块"
        );
        assert!(
            !ctx.reasoning_stream_seen,
            "本形态不得走结构化 reasoning"
        );
        let thinking_index = ctx
            .thinking_block_index
            .expect("嗅探必须分配 thinking 块索引");

        all.extend(ctx.process_tool_use(&crate::kiro::model::events::ToolUseEvent {
            name: "execute_bash".into(),
            tool_use_id: "toolu_sniff".into(),
        ...
        let tool_start_pos = all
            ...
            .expect("tool_use 块必须 start");
        assert!(
            thinking_stop_pos < tool_start_pos,
            "content_block_stop(thinking) 必须排在 content_block_start(tool_use) 之前"
        );
    }
    ...
    /// P0-1：8 个内置工具完整 **Kiro 形态** 参数，经 `process_tool_use` 还原后不得误杀。
    /// `tool_required_fields` 按 Kiro 名；`tool_block_names` 由真实 stop 路径写入。
    #[test]
    fn bug_c_mapped_builtin_tools_complete_kiro_params_do_not_fail() {
        crate::anthropic::set_tool_stream_align_failure(true);
        // (kiro_name, client_name, kiro_json, required, client_key that must appear in delta)
        let cases: &[(&str, &str, &str, &[&str], &str)] = &[
            ...
            (
                "str_replace",
                "Edit",
                r#"{"path":"/tmp/a.txt","oldStr":"a","newStr":"b"}"#,
                &["path", "oldStr", "newStr"],
                "file_path",
            ),
            (
            ...
            (
                "read_file",
                "Read",
                r#"{"path":"/tmp/a.txt","explanation":"why"}"#,
                &["path", "explanation"],
                "file_path",
            ),
            (
        ...
        for (kiro_name, client_name, kiro_json, required, client_key) in cases {
            let mut map = HashMap::new();
            map.insert((*kiro_name).to_string(), (*client_name).to_string());
            ...
            let events = ctx.process_tool_use(&crate::kiro::model::events::ToolUseEvent {
                ...
                tool_use_id: format!("toolu_{client_name}"),
                input: (*kiro_json).to_string(),
                stop: true,
            });
            assert!(
                ctx.completion().is_ok(),
                "{client_name}/{kiro_name}: 完整 Kiro 参数不得置失败态，got {:?}",
                ctx.completion()
            );
            let partial = events
                .iter()
                ...
                .unwrap_or_else(|| panic!("{client_name}: 应发出 input_json_delta"))
                .data["delta"]["partial_json"]
                .as_str()
                .unwrap_or("");
            assert!(
                partial.contains(client_key),
                "{client_name}: delta 应含客户端键 {client_key}，实际={partial}"
            );
            assert!(
                !partial.contains("INVALID_TOOL_INPUT"),
                "{client_name}: 不得发出 INVALID_TOOL_INPUT"
            );
        }
    }

    /// P0-1：Bash 缺 `command` 经 `process_tool_use` 仍判 Bug C（不只测谓词）。
    #[test]
    fn bug_c_bash_missing_command_still_fails() {
        crate::anthropic::set_tool_stream_align_failure(true);
        let mut map = HashMap::new();
