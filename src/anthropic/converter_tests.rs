    ...
    /// Prefill：末尾 assistant 被丢掉，当前轮仍是最后一条 user，prefill 正文不进 history。
    #[test]
    fn convert_request_drops_trailing_assistant_prefill() {
        ...
        let req = MessagesRequest {
            ...
            messages: vec![
                AnthropicMessage {
                    role: "user".to_string(),
                    content: serde_json::json!("hello user"),
                },
                AnthropicMessage {
                    role: "assistant".to_string(),
                    content: serde_json::json!("kept history reply"),
                },
                AnthropicMessage {
                    role: "user".to_string(),
                    content: serde_json::json!("current turn"),
                },
                AnthropicMessage {
                    role: "assistant".to_string(),
                    content: serde_json::json!("partial prefill must drop"),
                },
            ],
            stream: false,
            system: None,
        ...
        let result = convert_request(&req).expect("prefill 截断后转换应成功");
        assert_eq!(
            result
                .conversation_state
                .current_message
                ...
                .content,
            "current turn",
            "截断后 current_message 必须是最后一条 user，不是 prefill"
        );

        let mut saw_prefill = false;
        let mut saw_kept_assistant = false;
        for msg in &result.conversation_state.history {
            if let Message::Assistant(a) = msg {
                let c = &a.assistant_response_message.content;
                if c.contains("partial prefill must drop") {
                    saw_prefill = true;
                }
                if c.contains("kept history reply") {
                    saw_kept_assistant = true;
                }
            }
        }
        assert!(
            !saw_prefill,
            "trailing assistant prefill 不得进入 history"
        );
        assert!(
            saw_kept_assistant,
            "截断只丢末尾 prefill，中间 assistant 仍应在 history"
        );
    }
