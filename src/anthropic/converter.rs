...
pub fn convert_request(req: &MessagesRequest) -> Result<ConversionResult, ConversionError> {
    ...
    // 2.5. 预处理 prefill：末尾非 user（典型是 assistant prefill）截断到最后一条 user。
    // Claude 4.x 已弃用 assistant prefill，Kiro API 也不支持。
    // 响应头 `x-kirostudio-prefill-dropped` 未接：要把它送到 `/v1/messages`
    // 出口必须改 `handlers_dispatch` / 双端点 builder，本 MINOR 不碰那条路径。
    let messages: &[_] = if req.messages.last().is_some_and(|m| m.role != "user") {
        tracing::warn!(
            dropped_role = req.messages.last().map(|m| m.role.as_str()).unwrap_or(""),
            "末尾非 user 消息（assistant prefill）已丢弃，截断到最后一条 user"
        );
...
/// 构建请求级 `additionalModelRequestFields`。
///
/// - Claude 白名单 + native 开关：`output_config.effort`
/// - catalog GPT 族：`reasoning.effort`（不依赖 native 开关，不改 absorb）
fn build_additional_model_request_fields(
    ...
    if is_gpt_family(model_id) {
        ...
        let effort = select_native_reasoning_effort(req, EFFORTS_WITH_XHIGH);
...
fn is_gpt_family(model_id: &str) -> bool {
    super::model_catalog::resolve(model_id)
        .is_some_and(|r| r.spec.family == super::model_catalog::Family::Gpt)
}
...
fn generate_thinking_prefix_for_model(req: &MessagesRequest, model_id: &str) -> Option<String> {
    if build_additional_model_request_fields(req, model_id).is_some() {
        return None;
    }
