...
//! 用例，并在守卫里恢复原值（与 ENV_NOISE_TEST_LOCK 同款）。
    ...
    /// GPT 族走 `reasoning.effort`，不依赖 native 开关，也不出 `output_config`。
    #[test]
    fn gpt_family_uses_reasoning_effort_channel() {
        let _g = NativeEffortGuard::with(false);
        let req = mk_req(Some(enabled_thinking(32_000)), None);
        let fields = build_additional_model_request_fields(&req, "gpt-5.6-sol")
            .expect("GPT 族应走 reasoning 通道");
        assert!(fields.output_config.is_none(), "GPT 不得写 Claude output_config");
        assert_eq!(
            fields.reasoning.as_ref().map(|r| r.effort.as_str()),
            Some("high")
        );
        assert!(
            generate_thinking_prefix_for_model(&req, "gpt-5.6-sol").is_none(),
            "GPT reasoning 通道下抑制 XML thinking 标签"
        );
        let off = mk_req(
        ...
        assert!(build_additional_model_request_fields(&off, "gpt-5.6-sol").is_none());
    }
