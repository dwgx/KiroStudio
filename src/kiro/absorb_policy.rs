...
impl AbsorbPolicy {
    ...
    pub(super) fn class_allowed(&self, class: AbsorbClass) -> bool {
        match class {
    ...
    pub(super) fn class_deadline(
        ...
        if matches!(class, AbsorbClass::SwapWindow) && !self.swap_budget.is_zero()
        {
    ...
    /// 「这一类可以睡更久」的表态。
    pub(super) fn class_max_delay(&self, class: AbsorbClass) -> Duration {
        if matches!(class, AbsorbClass::SwapWindow) && !self.swap_budget.is_zero()
        {
            let ladder_top =
    ...
    pub(super) fn required_wait(&self, class: AbsorbClass, round: u32) -> Duration {
        match class {
    ...
    /// 算出的剩余秒数，无需 HTTP 头往返）。
    pub(super) fn backoff(&self, class: AbsorbClass, round: u32) -> Duration {
    ...
    pub(super) fn backoff_is_truncated(&self, class: AbsorbClass, round: u32) -> bool {
        matches!(class, AbsorbClass::PoolCooldown(_))
