...
const DEFAULT_TABLE: &[(&str, u16, &str, &str, Option<u64>)] = &[
    ...
    // B2 quota_exhausted_all（全池月度配额耗尽）→ 402 billing_error。
    (
        "quota_exhausted",
        402,
        "billing_error",
        "号池内所有凭据的月度请求配额均已耗尽（当月内不可恢复）。排障：①面板查看各凭据用量；②等待配额周期重置——跨月后自动恢复，无需人工介入；③为号池补充新凭据可立即恢复。",
        None,
    ),
